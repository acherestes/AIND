    ...
    def select(self):
        ...
        for state in range(self.min_n_components, self.max_n_components + 1):
            try:
                ...
                no_features = state ** 2 + 2 * state * model.n_features - 1
                current_BIC = -2 * model.score(self.X, self.lengths) + no_features * np.log(len(self.X))
                if BIC > current_BIC:
                    BIC = current_BIC
                    best_model = model
            except:
            ...
            try:
                ...
                for word in self.hwords:
                    if word != self.this_word:
                        X, lens = self.hwords[word]
                        scores += model.score(X, lens)
                        count += 1
                if count:
                    current_DIC = model.score(self.X, self.lengths) - scores / count
                else:
                    current_DIC = float("-inf")
                if current_DIC > DIC:
                    DIC = current_DIC
                    best_model = model
            except:
        ...
        for state in range(self.min_n_components, self.max_n_components + 1):
            ...
            for train_index, test_index in split_data.split(self.sequences):
                try:
                    ...
                    model = GaussianHMM(n_components=state, covariance_type="diag", n_iter=1000,
