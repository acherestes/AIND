...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if new_node.prenodes.issubset(self.s_levels[level]):
                # use .issubset function because both are sets (i.e. check that new_node intersect s_levels[level] == new_node)
                a_set.add(new_node)
                for prenode in self.s_levels[level]:
                    if prenode in new_node.prenodes:
                        prenode.children.add(new_node)
                        new_node.parents.add(prenode)
        self.a_levels.append(a_set)
    ...
    def add_literal_level(self, level):
        # add a literal level set and connect it to its respective parents and children
        s_set = set()
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.effnodes:
            for precond in node_a2.prenodes:
                if effect.symbol == precond.symbol:
                    if effect.is_pos != precond.is_pos:
                        return True
        for precond in node_a2.effnodes:
            for effect in node_a1.prenodes:
    ...
    def h_levelsum(self) -> int:
        ...
        for state in self.s_levels:
            for literal in state:
                if literal.symbol in goals and literal.is_pos:
                    level_sum += self.s_levels.index(state)
